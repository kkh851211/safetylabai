...
export function AdminPermissionManagementScreen() {
  ...
  const handlePermissionChange = (id: string, value: string) => {
    setAccounts((prev) =>
      prev.map((account) =>
        account.id === id ? { ...account, permissionLevel: value } : account
      )
    );
  };

  const handleHeadquartersChange = (id: string, value: string) => {
    setAccounts((prev) =>
      prev.map((account) =>
        account.id === id ? { ...account, headquarters: value } : account
      )
    );
  };
                    ...
                    <td className="px-6 py-4">
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) =>
                            handlePermissionChange(account.id, e.target.value)
                          }
                    ...
                    <td className="px-6 py-4">
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) =>
                            handleHeadquartersChange(account.id, e.target.value)
                          }
                          className="w-full pl-4 pr-10 py-2 border rounded-lg focus:outline-none focus:ring-2 appearance-none cursor-pointer bg-white text-sm"
                          style={{
                            borderColor: "var(--color-border-default)",
